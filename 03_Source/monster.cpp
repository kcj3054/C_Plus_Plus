...
void monster::set_point(coordinate point)
{
	point = point;
}

coordinate monster::get_point()
{
	return point;
}

void monster::move(coordinate dir, int distance)
{
	cout << '\n' << "Monster Move" << '\n';
	cout << "몬스터가 " << coordinate::get_direction_string(dir) 
		<< "방향으로 " << distance << "만큼 이동하였습니다" << endl;
}
